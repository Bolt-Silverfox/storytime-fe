...
// Normalized narration voice shape returned by the backend's /voice/* routes
// (matches green — the mobile + web reader consume displayName/previewUrl/
// voiceAvatar, not the raw ElevenLabs fields).
...
export const uploadAvatarService = async (
  ...
  try {
    ...
    if (
      ...
      throw {
        message: 'Avatar upload returned an unexpected response',
        status: null,
      };
    }
...
// `isPremium` gates voice switching; the route is @OptionalAuth so guests get
// `isPremium: false`. Defaults to non-premium on any error so voice switching
// stays locked unless the server explicitly grants it.
...
// NOTE: green's per-paragraph reader consumes its own StoryAudioBatch /
// StoryAudioParagraph shapes (with read-along text + failed-paragraph retry) via
// startStoryAudioBatchService / getStoryAudioBatchStatusService, appended at the
// end of this file. Blue's SSE transport (getStoryAudioBatchStatus below +
// lib/story-audio-events.ts) is retained so the push-based path can be
// re-layered later.
...
// ----- Story browsing / quota / library (ported from green; endpoints already
// match the blue backend). Error handling follows blue's style: no AxiosError
// import — duck-type on `error.response`. -----
...
// ----- Per-paragraph story audio (green's reader transport). Kicks off a TTS
// batch and polls for completed paragraphs, unifying the differing POST /
// status response shapes. Error handling duck-types on `error.response`
// (blue does not import AxiosError). -----
